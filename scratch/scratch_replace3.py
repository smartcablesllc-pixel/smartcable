...
for line in lines:
    if '<div class="channel-card animate-on-scroll"' in line:
        # Extract channel name
        parts = line.split('data-channel-name="')
        if len(parts) > 1:
            current_channel = parts[1].split('"')[0]
    
    if '<svg' in line and current_channel:
        # We start an svg block
        in_svg = True
        # Let's write the image tag instead
        url = logos.get(current_channel, "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/300px-No_image_available.svg.png")
        alt_text = f"{current_channel.replace('&amp;', '&')} Official Logo"
        new_lines.append(f'              <img src="{url}" alt="{alt_text}" class="channel-logo" loading="lazy" width="60" height="60">\n')
    
    if in_svg:
        if '</svg>' in line:
            in_svg = False
        continue # Skip all svg lines
