...
for pid, url in verified_images.items():
    ...
    # Card starts with id="pid"
    pattern = r'(id="' + re.escape(pid) + r'"[^>]*>.*?<img class="product-card__img product-img" src=")([^"]*)(")'
    def repl(m):
        global updated
        updated += 1
...
print(f"Successfully updated {updated} / {len(verified_images)} product card images in index.html!")
