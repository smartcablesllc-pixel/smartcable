...
print(f"Checking {len(all_32)} items...")
failures = []
for pid, url in all_32.items():
    if url.startswith('images/'):
        if os.path.exists(url):
            print(f"[OK LOCAL] {pid:25s} -> {url} ({os.path.getsize(url)} bytes)")
        else:
            print(f"[FAIL LOCAL] {pid:25s} -> {url} not found!")
            failures.append(pid)
    else:
        ...
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                sz = len(resp.read())
                print(f"[OK HTTP]  {pid:25s} -> {resp.status} {resp.headers.get('Content-Type')} ({sz} bytes)")
        except Exception as e:
            print(f"[FAIL HTTP] {pid:25s} -> {e}")
            failures.append(pid)

print(f"\nTotal: {len(all_32)}, Failures: {len(failures)}")
